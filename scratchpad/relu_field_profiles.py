"""relu vs Φ on the DPA stage: the memory-axis field profile F₀/κ₀ (well test = sign change + → −),
...
ARMS = [("nocue", "results/dual/sweep_r2nocue", "Φ (lif), foundation"), ("fdrelu", "results/dual/sweep_r2fdrelu", "relu, no reg"),
        ("rr01", "results/dual/sweep_r2rr01", "relu + L2 w=0.01"), ("rr1", "results/dual/sweep_r2rr1", "relu + L2 w=0.1"),
        ("rrb01", "results/dual/sweep_r2rrb01", "relu + L2 0.01 + biases")]
...
fig.suptitle("DPA stage: memory-axis field profile (row 1: a well = sign change + → −; escape = stays > 0), κ₀ over the trial (row 2), rate scale (row 3)", fontsize=11)
fig.tight_layout()
out = "/home/leon/rnn/results/figures/sweep_r2rr01/field_profiles_relu_vs_lif"; os.makedirs(os.path.dirname(out), exist_ok=True)
