...
ARMS = [("nocue",  "results/dual/sweep_r2nocue",  "Φ (lif)\nwhole-delay hold"),
        ("w5lif",  "results/dual/sweep_r2w5lif",  "Φ (lif)\n0.5 s terminal hold"),
        ("fdrelu", "results/dual/sweep_r2fdrelu", "relu\nwhole-delay hold"),
        ("w5relu", "results/dual/sweep_r2w5r",    "relu\n0.5 s terminal hold"),
        ("rr01",   "results/dual/sweep_r2rr01",   "relu + L2 0.01\nwhole-delay hold"),
        ("w5rl2",  "results/dual/sweep_r2w5r",    "relu + L2 0.01\n0.5 s terminal hold")]
...
fig.suptitle("w5 — A/B hold over the WHOLE delay vs the LAST 0.5 s (pairs of columns). Memory-axis field profile (row 1: a well = sign change + → −; escape = stays > 0), κ₀ over the trial (row 2), rate scale (row 3)", fontsize=11)
fig.tight_layout()
out = "/home/leon/rnn/results/figures/sweep_r2w5r/field_profiles_w5"; os.makedirs(os.path.dirname(out), exist_ok=True)
