...
ARMS = [("nocue",  "results/dual/sweep_r2nocue", "Φ (lif) reference\nmemory_lambda 3.0"),
        ("w5relu", "results/dual/sweep_r2w5r",   "relu, from ABOVE\nλ⁺ init 1.50"),
        ("sc12",   "results/dual/sweep_r2sc1",   "relu, from BELOW\nλ⁺ init 0.60"),
        ("sc16",   "results/dual/sweep_r2sc1",   "relu, from BELOW\nλ⁺ init 0.80"),
        ("sc18",   "results/dual/sweep_r2sc1",   "relu, from BELOW\nλ⁺ init 0.90")]
...
fig.suptitle("Subcritical init — relu approached from ABOVE (lambda+ 1.50) vs from BELOW (0.60/0.80/0.90). Memory-axis field profile (row 1: a well = sign change + → −; escape = stays > 0), κ₀ over the trial (row 2), rate scale (row 3)", fontsize=11)
fig.tight_layout()
out = "/home/leon/rnn/results/figures/sweep_r2sc1/field_profiles_sc"; os.makedirs(os.path.dirname(out), exist_ok=True)
