...
| `sweep_lif_rulesym` (2026-09-21) | `rulesym_afc` / `_afc_tied` / `_gng` / `_gng_tied` / `_afc_dec` / `_afc_klein` / `_gng_dec` / `_gng_klein` ×4 | rule stage only (epochs 0/150/0), both modes free, cue on its own channel (input_size 7) | validation of the group theory on the rule task alone (`ring_lowerplane_log` §38): every exact claim holds in every seed — tied −I → exact antipodal orbits (fixed points and populations), cross-overlaps large under −I vs 0.000 under diag(+1,−1)/V, tied nets learn both objectives (0.98–1.00), free two-sided nets keep the whole group with the memory on the κ₁ axis; the soft claim (one-sided cost shows in the autonomous field) was wrong — the break lands in the input columns. 2/4 tied one-sided nets hold the memory on a slowly rotating ring |
| `sweep_lif_rulesym` `afc2_*` (2026-09-21) | `afc2_free` / `_inv` (−I) / `_dec` (diag(+1,−1)) / `_mem` (diag(−1,+1) with L↔R — NOT a symmetry) / `_klein` ×4 | Leon's delayed 2AFC (`rule_timing="2afc"`: stim 2–3 s, delay, cue 6–7 s, response to trial end), 150 epochs | §39b: free 0.998–1.000 with the memory on the κ₁ axis (0, ±0.9); −I 0.98–0.999, residual 0.000, antipodes at a free angle, seed 2 a limit cycle (radius 0.85, period ≈ 30 s); dec 0.98–0.99, residual 0.000, mirror pair; **mem 0.48–0.53 (chance 4/4, as predicted)**; klein 0.96–0.99 in 3 seeds (0.43 = slow learner), all residuals 0.000, κ₀ ≡ 0 on every trial, memory held on the invariant κ₁ axis. Readout `afc2_readout.tsv` in the sweep dir |
| `sweep_lif_symdpa` `symdpa_inv` / `symdpa_test` (2026-09-21, curricula RUNNING) | σ₂ = −I (A↔B alone) / σ₃ = diag(+1,−1) (C↔D alone) ×4, bias tied | all three, symmetry in DPA only | DPA ckpt (§36e): σ₂ → residual 0.00, antipodal pair ON the line (±1, 0), σ₁ residual 0.39, lattice tilted by the free cross-overlaps (J₁₀ up to −0.95); σ₃ → residual 0.00, pair on the κ₀ axis + vertical decision pair (0, ±1), ⟨n₁⟩ = J₀₁ = J₁₀ = 0 exactly ⇒ indistinguishable from the whole group (σ₃ closes every channel DPA uses to break σ₁/σ₂). Expert ckpt pending |
| `sweep_lif_recipe7_bfix` (QUEUED after symdpa_inv/test) | `recipe7` ×8 | all three, free, **bias frozen in Dual** (post-review code) | the check of the 2026-09-21 freeze fix against `sweep_lif_recipe7` (depth, 6/8, ledger Dual column) |

**New machinery.** `RunConfig.symmetry` (`"pair"` / `"test"` / `"inv"` / `"klein"`; rule-task kinds `"gng"` (−I), `"gng_dec"` (diag(+1,−1)), `"gng_mem"` (diag(−1,+1)), `"gng_klein"` — all tie `wi.bias` since 2026-09-21) and `symmetry_stages`; `rule_timing="2afc"` + `afc_response_to_end` for the delayed 2AFC; `freeze_rank0_gng`;
